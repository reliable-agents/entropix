...
def attention(x: torch.Tensor, layer_weights: LayerWeights, model_params, cur_pos: int, layer_idx: int, freqs_cis: torch.Tensor, kvcache: KVCache, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, KVCache, torch.Tensor]:
    ...
    xq, xk = apply_rotary_emb(xq, xk, freqs_cis=freqs_cis, dtype=xq.dtype) # Fix from: https://github.com/xjdr-alt/entropix/pull/39
    ...
    output = torch.matmul(scores.to(values.dtype), values) # Fix from: https://github.com/xjdr-alt/entropix/pull/39
